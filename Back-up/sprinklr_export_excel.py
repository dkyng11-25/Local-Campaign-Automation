...
# 필요하신 scope에 해당하는 날짜 기입 + Enter 하시면 실행 됩니다.
# =========================================================
...
def build_time_range_from_dates(
    start_date_str: str,
    end_date_str: str
) -> tuple[int, int, str, str]:
    """
    날짜만 입력받아서 Sprinklr payload용 startTime/endTime milliseconds로 변환

    규칙:
        start_date 18:00:00부터
        end_date   16:00:59까지

    입력 예시:
        start_date_str = "2026-07-07"
        end_date_str   = "2026-07-08"
    ...
    start_datetime_str = f"{start_date_str} 18:00:00"
    end_datetime_str = f"{end_date_str} 16:00:59"
    ...
    if start_time_ms >= end_time_ms:
        raise ValueError(
            f"Invalid time range. startTime must be earlier than endTime. "
...
def main() -> None:
    ...
    start_date = input("Enter start datetime (YYYY-MM-DD): ").strip()
    end_date = input("Enter end datetime (YYYY-MM-DD): ").strip()

    naming_date = end_date.replace("-", "")
    ...
    start_time_ms, end_time_ms, data_cut_start, data_cut_end = build_time_range_from_dates(
