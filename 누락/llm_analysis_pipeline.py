...
# prompts와 config는 프로젝트 루트의 공용 리소스를 사용한다.
...
GEMINI_MAX_WORKERS = int(os.getenv("GEMINI_MAX_WORKERS", "10"))
...
def run_llm_pipeline(
    ...
    try:
        ...
        pending_futures: dict[Any, tuple[int, LLMInputSet]] = {}
        ...
        try:
            for current_position, input_set in enumerate(input_sets, start=1):
                ...
                if max_rows is not None and attempted_count >= max_rows:
                    call_result = LLMCallResult(
                        dataframe_index=input_set.dataframe_index,
            ...
            for future in as_completed(pending_futures):
                _current_position, input_set = pending_futures[future]
                ...
                if (
                    ...
                    safe_error_message = (
                        str(call_result.api_error_message)
                        .encode(
                            console_encoding,
...
def resolve_execution_paths(
    ...
    # 1) CLI --input-date가 있으면 가장 먼저 사용한다.
    input_date = (
    ...
    # 2) run_pipeline.py가 날짜를 환경변수로 전달하는 경우도 지원한다.
    #    현재 누락 run_pipeline.py는 stdin 전달 방식을 사용하지만,
    #    환경변수 방식과도 호환성을 유지한다.
    environment_input_date = os.getenv(ENV_INPUT_DATE)
    if input_date is None and environment_input_date:
        input_date = validate_input_date(
            environment_input_date
        )
    ...
    # 중요:
    # 누락 run_pipeline.py는 `260831` 같은 날짜를 CLI 옵션이 아니라
    # 표준입력(stdin)으로 전달한다. 따라서 output 폴더를 resolve하기 전에
    # 날짜를 먼저 읽어야 한다.
    #
    # 단, --output-dir 또는 LOCAL_CAMPAIGN_OUTPUT_DIR가 명시되어 있으면
    # 해당 폴더명에서 날짜를 추론할 수 있으므로 먼저 input()을 소비하지 않는다.
    has_explicit_output_dir = (
        args.output_dir is not None
        or bool(os.getenv(ENV_OUTPUT_DIR))
    )

    if (
        input_date is None
        and defaults_required
        and not has_explicit_output_dir
    ):
        input_date = validate_input_date(
            input(
                "조회 날짜를 입력하세요 (YYMMDD): "
    ...
    # 4) 이제 날짜가 확보된 상태에서 실행 output 폴더를 확정한다.
    #    누락 기본 경로:
    #      프로젝트루트/누락/output_누락/{YYMMDD}_누락
    (
    ...
    # --output-dir만 지정한 경우에는 폴더명에서 날짜를 가져온다.
    if input_date is None:
        input_date = directory_date

    # 방어적 fallback: 모든 기본 경로가 필요한데 여기까지 날짜를
    # 확정하지 못한 경우에만 stdin을 읽고 output 폴더를 다시 resolve한다.
    ...
    if input_date is None:
        raise ValueError(
            "input_date를 파일명, 환경변수 또는 실행 output 폴더에서 "
